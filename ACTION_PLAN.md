...
| 8 | ✅ **MCP server** — SHIPPED 2026-07-01 (branch `feat/mcp-server`). `services/mcp-server/inmyhead_mcp.py` (FastMCP, stdio) exposes 4 tools to Claude Code/Cursor/any MCP client: `search_knowledge` (grounded answer+sources), `search_documents` (raw passages), `list_documents`, `related_documents`. Thin bridge over the ai-engine HTTP API — fully local. Verified: 4 tools register + call the live KB (listed 5 docs, found related). README has the Claude Code / Cursor config. Richer than the raw Qdrant MCP — exposes full RAG, not just vectors. | S |
