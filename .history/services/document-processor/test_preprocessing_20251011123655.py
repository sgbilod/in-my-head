...
def test_deduplicator():
    ...
    texts = [
        ...
        "This is the first text.",  # Exact duplicate
    ]
    
    duplicates = deduplicator.find_duplicates(texts)
    assert len(duplicates) > 0
    print(f"  ✓ Exact duplicates: Found {len(duplicates)} pairs")
