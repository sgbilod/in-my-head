...
class TestJobManager:
    ...
    @patch("jobs.tasks.process_document_task")
    def test_submit_document(self, mock_task, job_manager):
        """Test document submission."""
        # Import the task to patch correctly
        ...
        # Submit document
        with patch.object(tasks, "process_document_task", mock_task):
    ...
    def test_submit_batch(self, mock_task, job_manager):
        ...
        assert mock_task.apply_async.call_count == 3    @patch("jobs.job_manager.AsyncResult")
...
class TestTaskExecution:
    ...
    def test_parse_document_task(self):
        ...
        # Mock ParserFactory at module level
        with patch("jobs.tasks.ParserFactory") as mock_factory:
            # Mock parser
            mock_parser = Mock()
            mock_result = Mock()
            ...
            # Execute task
            result = parse_document_task.__wrapped__(
                parse_document_task, "test.pdf"
            )
    ...
    def test_preprocess_text_task(self):
        """Test preprocess_text_task."""
        from jobs.tasks import preprocess_text_task
        
        # Mock at module level
        with patch("jobs.tasks.TextCleaner") as mock_cleaner_class, \
             patch("jobs.tasks.TextChunker") as mock_chunker_class:
            ...
            result = preprocess_text_task.__wrapped__(
                preprocess_text_task, parse_result
            )
